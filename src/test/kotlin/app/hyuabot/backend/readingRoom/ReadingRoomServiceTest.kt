...
class ReadingRoomServiceTest {
    ...
    fun testGetReadingRoomList() {
        ...
        val readingRoomList = readingRoomService.getReadingRoomList(null)
    ...
    fun testGetReadingRoomListWithKeyword() {
        val now = ZonedDateTime.now(ZoneId.of("Asia/Seoul"))
        whenever(readingRoomRepository.findByNameContaining("열람실")).thenReturn(
            listOf(
                ReadingRoom(
                    id = 1,
                    name = "열람실 A",
        ...
        val readingRoomList = readingRoomService.getReadingRoomList("열람실")
        assertEquals(2, readingRoomList.size)
        assertEquals("열람실 A", readingRoomList[0].name)
        assertEquals("열람실 B", readingRoomList[1].name)
        assertEquals(1, readingRoomList[0].campusID)
