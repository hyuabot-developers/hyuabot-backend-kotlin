...
class ContactCategoryController {
    ...
    @Operation(
        ...
        requestBody =
            io.swagger.v3.oas.annotations.parameters.RequestBody(
                content = [Content(schema = Schema(implementation = ContactCategoryRequest::class))],
                description = "수정할 전화부 카테고리 정보",
            ),
    )
    @ApiResponses(
        ApiResponse(
            responseCode = "200",
            description = "전화부 카테고리 수정 성공",
            content = [Content(schema = Schema(implementation = ContactCategoryResponse::class))],
        ),
        ApiResponse(
            responseCode = "404",
            description = "전화부 카테고리 수정 실패 - 존재하지 않는 ID",
            content = [
                Content(
                    schema = Schema(implementation = ResponseBuilder.Message::class),
                    examples = [
        ...
        ApiResponse(
            responseCode = "409",
            description = "전화부 카테고리 수정 실패 - 중복된 이름",
            content = [
                Content(
                    schema = Schema(implementation = ResponseBuilder.Message::class),
                    examples = [
    ...
    fun updateContactCategory(
        ...
        try {
            val category = service.updateContactCategoryById(seq, request)
            return ResponseBuilder.response(
                status = HttpStatus.OK,
                body =
                    ContactCategoryResponse(
            ...
            logger.error("Unexpected error occurred while updating contact category", e)
            return ResponseBuilder.response(
                status = HttpStatus.INTERNAL_SERVER_ERROR,
                body = ResponseBuilder.Message("INTERNAL_SERVER_ERROR"),
            )
