...
class NoticeCategoryController {
    ...
    @ApiResponses(
        ...
        ApiResponse(
            responseCode = "404",
            description = "공지사항 카테고리 수정 실패 - 존재하지 않는 ID",
            content = [
                Content(
                    schema = Schema(implementation = ResponseBuilder.Message::class),
                    examples = [
        ...
        ApiResponse(
            responseCode = "409",
            description = "공지사항 카테고리 수정 실패 - 중복된 이름",
            content = [
                Content(
                    schema = Schema(implementation = ResponseBuilder.Message::class),
                    examples = [
    ...
    fun updateNoticeCategory(
        @PathVariable seq: Int,
        @RequestBody request: NoticeCategoryRequest,
    ): ResponseEntity<*> {
        try {
            val category = service.updateNoticeCategoryById(seq, request)
            return ResponseBuilder.response(
            ...
            logger.error("Unexpected error occurred while updating notice category", e)
            return ResponseBuilder.response(
